...
function buildReciterGrid() {
  ...
  RECITERS.forEach((r, i) => {
    ...
    card.dataset.i = i;
    card.innerHTML = `
      <div class="reciter-avatar">${r.emoji}</div>
...
function selectReciter(idx) {
  ...
  // If a surah is loaded, reload it with new reciter
...
function buildAudioSurahList() {
  ...
  SURAH_IT.forEach((it, i) => {
    ...
    row.innerHTML = `
      ...
      <div class="as-info">
        <div class="as-it">${it}</div>
        <div class="as-ar">${ar}</div>
      </div>
      <div class="as-play-icon">▶</div>
    `;
...
function loadAudio(surahNum, autoplay) {
  ...
  // Use Islamic Network CDN: /quran/audio-surah/128/{edition}/{surah}.mp3
...
async function renderPrayers() {
  ...
  PRAYERS_DEF.forEach(p => {
    ...
    // Use dynamic time if available, else fallback to defined string
...
async function renderStats() {
  ...
  document.getElementById('statsRow').innerHTML = `
    <div class="stat-pill"><div class="sp-val">${pct}%</div><div class="sp-lbl">Completamento</div></div>
    <div class="stat-pill"><div class="sp-val">${perfect}</div><div class="sp-lbl">Giorni perfetti</div></div>
    <div class="stat-pill"><div class="sp-val">${total}</div><div class="sp-lbl">Preghiere totali</div></div>
    <div class="stat-pill"><div class="sp-val">${today}</div><div class="sp-lbl">Giorni tracciati</div></div>
  `;
}
...
function initTasbih() {
  ...
  DHIKR.forEach((d, i) => {
    ...
    chip.dataset.i = i;
    // Show Italian transliteration first, Arabic smaller below
...
function simulateAnswer(q) {
  ...
  return `<p>Ottima domanda su <em>"${q}"</em>.</p>
    <p>Per una risposta AI personalizzata inserisci la tua <strong>Gemini API Key</strong> nella barra sopra.</p>
    <p>Principi generali islamici:<br>
    • <strong>Dubbio:</strong> applica la precauzione (ihtiyat) — meglio evitare<br>
    • <strong>Necessità:</strong> la darura può rendere permesso ciò che è normalmente vietato<br>
    • <strong>Importante:</strong> consulta uno studioso qualificato</p>`;
}
