...
function setHijri() {
try {
const fmt = new Intl.DateTimeFormat(‘ar-SA-u-ca-islamic’, { day: ‘numeric’, month: ‘short’, year: ‘numeric’ });
document.getElementById(‘hijriPill’).textContent = fmt.format(new Date());
} catch { document.getElementById(‘hijriPill’).textContent = ‘’; }
}
setHijri();
...
row.innerHTML = `<div class="s-num">${n}</div><div class="s-info"><div class="s-it">${it}</div><div class="s-ar">${ar}</div></div><div class="s-meta"><div class="s-meta-v">${m.v}v</div><div class="s-meta-c">${m.c}</div></div>`;
...
el.innerHTML = `<div class="verse-ar">${v.text}</div><div class="verse-bottom"><div class="verse-n">${v.numberInSurah}</div><div class="verse-tr">${itText || '<em style="color:var(--text3)">Traduzione non disponibile</em>'}</div></div>`;
feed.appendChild(el);
});
} catch {
...
document.querySelectorAll(’.reciter-card’).forEach((c, i) => c.classList.toggle(‘active’, i === idx));
...
document.getElementById(‘prevSurahBtn’).addEventListener(‘click’, () => { if (currentAudioSurah > 1) loadAudio(currentAudioSurah - 1, true); });
document.getElementById(‘nextSurahBtn’).addEventListener(‘click’, () => { if (currentAudioSurah < 114) loadAudio(currentAudioSurah + 1, true); else loadAudio(1, true); });
...
audio.addEventListener(‘loadedmetadata’, () => { document.getElementById(‘playerDurEl’).textContent = formatTime(audio.duration); });
audio.addEventListener(‘ended’, () => { isPlaying = false; playBtn.textContent = ‘▶’; playBtn.classList.remove(‘playing’); if (currentAudioSurah < 114) loadAudio(currentAudioSurah + 1, true); });
audio.addEventListener(‘play’,  () => { isPlaying = true;  playBtn.textContent = ‘⏸’; playBtn.classList.add(‘playing’); });
audio.addEventListener(‘pause’, () => { isPlaying = false; playBtn.textContent = ‘▶’; playBtn.classList.remove(‘playing’); });
slider.addEventListener(‘input’, () => { if (audio.duration) audio.currentTime = (slider.value / 100) * audio.duration; });
}
...
audio.src = url;
audio.load();
audio.onerror = () => { document.getElementById(‘playerSurahIt’).textContent = “⚠️ Errore caricamento”; document.getElementById(‘playerReciterName’).textContent = “Prova un altro lettore”; };
...
document.getElementById(‘prayerDate’).textContent = new Date().toLocaleDateString(‘it-IT’, { weekday: ‘long’, day: ‘numeric’, month: ‘long’ });
...
card.innerHTML = `<div class="p-emoji">${p.emoji}</div><div class="p-info"><div class="p-it-title">${p.it}</div><div class="p-time-desc">${displayTime}</div></div><div class="p-check">${isDone ? '✓' : ''}</div>`;
card.addEventListener(‘click’, async () => { data[p.id] = !data[p.id]; await window.RahmaDB.savePrayers(todayKey(), data); await renderPrayers(); });
card.addEventListener(‘keydown’, async e => { if (e.key === ‘Enter’ || e.key === ’ ’) { e.preventDefault(); data[p.id] = !data[p.id]; await window.RahmaDB.savePrayers(todayKey(), data); await renderPrayers(); } });
...
} catch (e) { locBtn.innerHTML = ‘⚠️ Errore Rete’; }
}, () => { locBtn.innerHTML = ‘⚠️ Posizione Negata’; });
} else { locBtn.innerHTML = ‘⚠️ Non supportato’; }
});
...
col.innerHTML = `<div class="wd-name">${names[d.getDay()]}</div><div class="wd-bar-wrap"><div class="wd-bar-fill" style="height:${pct}%"></div></div><div class="wd-count">${cnt}</div>`;
...
dkCount = saved.count; dkSession = saved.session; dkHistory = saved.history;
...
window.RahmaDB.getTasbihData(dkIdx).then(saved => { dkCount = saved.count; dkSession = saved.session; dkHistory = saved.history; updateTasbihDisplay(); });
}
...
const avatar = role === ‘bot’ ? `<div class="msg-avatar bot-av">R</div>` : `<div class="msg-avatar user-av">👤</div>`;
...
body: JSON.stringify({ contents: [{ parts: [{ text: `${sys}\n\nDomanda: ${q}` }] }], generationConfig: { maxOutputTokens: 512, temperature: 0.7 } })
});
...
document.getElementById(‘chatSend’).addEventListener(‘click’, () => { const v = chatInput.value; chatInput.value = ‘’; chatInput.style.height = ‘auto’; sendChat(v); });
chatInput.addEventListener(‘keydown’, e => { if (e.key === ‘Enter’ && !e.shiftKey) { e.preventDefault(); const v = chatInput.value; chatInput.value = ‘’; chatInput.style.height = ‘auto’; sendChat(v); } });
...
openPrivacyBtn.addEventListener(‘click’, () => { privacyModal.classList.remove(‘hidden’); privacyModal.setAttribute(‘aria-hidden’, ‘false’); });
const closeModal = () => { privacyModal.classList.add(‘hidden’); privacyModal.setAttribute(‘aria-hidden’, ‘true’); };
